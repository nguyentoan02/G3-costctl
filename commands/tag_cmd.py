...
def _tag_s3(rid, tags):
    ...
    final_tags = [{"Key": key, "Value": value} for key, value in sorted(merged.items())]
    s3.put_bucket_tagging(Bucket=rid, Tagging={"TagSet": final_tags})
...
def run(args):
    ...
    rendered = ", ".join(f"{item['Key']}={item['Value']}" for item in tags)
    print(f"Applied {len(tags)} tag(s) to {args.type} {args.id}: {rendered}")
